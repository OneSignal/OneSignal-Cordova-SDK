...
The demo iOS project uses Capacitor's SPM layout:

- `ios/App/CapApp-SPM` is generated by Capacitor and references installed Capacitor plugins.
- The Cordova plugin is resolved from `node_modules/onesignal-cordova-plugin` through `CapApp-SPM/Package.swift`.
- The app target links `CapApp-SPM`.
- The notification service extension links `OneSignalExtension`.
- The widget extension links `OneSignalFramework` for the live activities APIs.
...
Then open `ios/App/App.xcodeproj` in Xcode.
...
Note: on-disk folder is `OneSignalWidget`, target name is `OneSignalWidgetExtension` (matching the Xcode target).
...
After syncing, open `ios/App/App.xcodeproj` in Xcode and verify:
...
If native config changes are not reflected, rerun `vp run setup:ios` and resolve packages again.
